...
int main()
{
    ...
    for(i=1;i<nDays;i++)
    {
        ...
        dpMatrix[i][2] = max(dpMatrix[i-1][0], dpMatrix[i-1][1]) + high;
        pathMatrix[i][2] = (dpMatrix[i-1][0] >= dpMatrix[i-1][1]) ? 0 : 1;
    }

    if(DEBUG)
        printDPMatrix(dpMatrix, nDays);

    printf("\nMaximum Calories consumed: %u", max( dpMatrix[nDays-1][0], max(dpMatrix[nDays-1][1], dpMatrix[nDays-1][2])));
    // Print the schedule
